...
检查浏览器 `localStorage.userInfo` 是否存在、是否包含正确的 `role` 字段。

### 2) 前端能打开但接口都失败

确认后端是否已启动在 `8080`，并检查前端请求地址是否仍指向本地。

### 3) SQL 执行报字段或索引已存在

迁移脚本设计为尽量幂等；若出现历史环境差异，请先备份数据库后手动核对表结构再执行。

## 许可证

如需开源发布，可补充 MIT/Apache-2.0 等许可证文件。